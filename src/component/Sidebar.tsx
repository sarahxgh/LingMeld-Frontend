...
import dashboardIcon from './images/Dashboard.png'; // Corrected path
...
const NavLink: React.FC<NavItem & { isActive: boolean; onClick: () => void }> = ({ href, icon, label, isActive, onClick }) => (
  <li>
    <a 
      href={href} 
      onClick={onClick}
      className={`flex gap-4 items-center mt-4 p-3 ${isActive ? 'bg-white rounded-2xl' : ''}`}
    >
      <div className={`flex justify-center items-center bg-white rounded-xl h-[35px] w-[35px] shadow-sm ${isActive ? '' : ''}`}>
        <img 
          loading="lazy" 
          src={icon} 
          alt={`${label} icon`} 
